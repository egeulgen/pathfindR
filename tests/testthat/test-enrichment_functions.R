...
test_that("hyperg_test returns a p value", {
  expect_is(
    tmp_p <- hyperg_test(letters[1:5], letters[2:5], letters),
    "numeric"
  )
  expect_true(tmp_p >= 0 & tmp_p <= 1)

  expect_is(
    tmp_p <- hyperg_test(letters[1:5], letters[2:10], letters),
    "numeric"
  )
  expect_true(tmp_p >= 0 & tmp_p <= 1)
})
