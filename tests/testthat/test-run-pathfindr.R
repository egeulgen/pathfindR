...
test_that("`run_pathfindR()` -- works as expected", {
  ...
  expected_messages <- paste(
    c(
      "The input looks OK", "Plotting the enrichment bubble chart",
      paste(c(
        paste0("Found ", nrow(example_pathfindR_output), " enriched terms\n"),
  ...
  expect_message(
    res <- run_pathfindR(input_data_frame, output_dir = out_dir),
    expected_messages
  )
...
test_that("`run_pathfindR()` argument checks work", {
  expect_error(
    run_pathfindR(input_data_frame, plot_enrichment_chart = "INVALID"),
    "`plot_enrichment_chart` should be either TRUE or FALSE"
  )
  expect_error(
    run_pathfindR(input_data_frame, list_active_snw_genes = "INVALID"),
