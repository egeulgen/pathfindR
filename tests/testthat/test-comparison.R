...
test_that("`combined_results_graph()` -- produces a ggplot object using the correct data", {
  ...
  expect_is(
    p <- combined_results_graph(combined_df, selected_terms = sel_terms),
    "ggplot"
  )
  ...
  expect_is(
    p <- combined_results_graph(combined_df, use_description = TRUE),
    "ggplot"
  )
...
test_that("`combined_results_graph()` -- argument checks work", {
  ...
  expect_error(combined_results_graph(combined_df, node_size = "INVALID"), paste0(
  ...
  expect_error(combined_results_graph(wrong_df, use_description = FALSE), paste(
    c(
      "All of",
  ...
  expect_error(combined_results_graph(wrong_df, use_description = TRUE), paste(
    c(
      "All of",
      paste(necessary_cols, collapse = ", "), "must be present in `results_df`!"
    ),
