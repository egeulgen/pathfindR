...
test_that("`score_terms()` -- returns score matrix", {
  ...
  expect_is(score_terms(
    enrichment_table = small_result, exp_mat = example_experiment_matrix,
    plot_hmap = FALSE
  ), "matrix")
  expect_is(score_terms(
    enrichment_table = small_result, exp_mat = example_experiment_matrix,
  ...
  expect_is(score_terms(
    enrichment_table = small_result, exp_mat = example_experiment_matrix,
    cases = colnames(example_experiment_matrix)[1:3], plot_hmap = TRUE
  ), "matrix")
})
...
test_that("`score_terms()` -- matches gene symbols correctly", {
  ...
  toy_exp_mat <- matrix(
    rnorm(40),
    nrow = 10, ncol = 4, dimnames = list(paste0("gene_", 1:10), paste0("subject_", 1:4))
  )
  expect_is(res_mat <- score_terms(
    enrichment_table = toy_result, exp_mat = toy_exp_mat,
    plot_hmap = FALSE
  ), "matrix")
  ...
  expect_is(res_mat <- score_terms(
    enrichment_table = toy_result2, exp_mat = toy_exp_mat,
    plot_hmap = FALSE
  ), "matrix")
...
test_that("`score_terms()` -- argument checks work", {
  expect_error(score_terms(
    enrichment_table = example_pathfindR_output, exp_mat = example_experiment_matrix,
  ...
  expect_error(
    score_terms(enrichment_table = list(), exp_mat = example_experiment_matrix),
  ...
  expect_error(
    score_terms(enrichment_table = tmp, exp_mat = example_experiment_matrix),
  ...
  expect_error(score_terms(
    enrichment_table = tmp, exp_mat = example_experiment_matrix,
  ...
  expect_error(
    score_terms(enrichment_table = example_pathfindR_output, exp_mat = list()),
...
test_that("duplicated term descriptions test", {
  ...
  expect_is(score_terms(
    enrichment_table = small_result, exp_mat = example_experiment_matrix,
    use_description = TRUE, plot_hmap = FALSE
  ), "matrix")
})
...
test_that("`plot_scores()` -- argument checks work", {
  ...
  mat <- matrix(1, nrow = 3, ncol = 2, dimnames = list(paste0("T", 1:3), c(
    "A",
    "B"
  )))
