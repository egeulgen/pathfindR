...
test_that("return_pin_path returns the absolute path to PIN file", {
  ...
  write.table(custom_sif, "./tmp_sif.sif",
    sep = "\t",
    col.names = FALSE, row.names = FALSE
  )
  expect_error(
    return_pin_path("./tmp_sif.sif"),
...
test_that("input_processing works", {
  ...
  expect_is(
    input_processing(RA_input[1:10, ],
      p_val_threshold = 0.05,
      pin_path = path2pin,
      human_genes = FALSE
    ),
...
test_that("annotate_pathway_DEGs adds input genes for each term", {
  ...
  ## Default
  expect_is(
    annotated_result <- annotate_pathway_DEGs(
      tmp_res,
      example_gene_data
    ),
    "data.frame"
  ...
  ## Custom
  expect_is(
    annotated_result <- annotate_pathway_DEGs(tmp_res,
      example_gene_data,
      gene_sets = "Custom",
