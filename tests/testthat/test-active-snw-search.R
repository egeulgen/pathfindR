...
test_that("`active_snw_search()` -- returns a list object", {
  ...
  expect_message(
    snw_list <- active_snw_search(input_for_search = input_data_frame),
...
test_that("`active_snw_search()` -- argument checks work", {
  ...
  # snws_file
  expect_error(snw_list <- active_snw_search(
  ...
  expect_error(
    active_snw_search(input_for_search = input_data_frame, search_method = "INVALID"),
    paste0("`search_method` should be one of ", paste(dQuote(valid_mets), collapse = ", "))
  )

  # silent_option
  expect_error(
    active_snw_search(input_for_search = input_data_frame, silent_option = "WRONG"),
    "`silent_option` should be either TRUE or FALSE"
  )

  expect_error(
    active_snw_search(input_for_search = input_data_frame, use_all_positives = "INVALID"),
...
test_that("`active_snw_search()` -- results are reproducible", {
  ...
  for (idx in 1:3) {
    seed <- seed_vals[idx]
    snw_lists[[idx]] <- active_snw_search(
      input_for_search = input_data_frame,
