test_that("`fetch_gene_sets()` -- can fetch all gene set objects", {
  ...
  for (gset_name in c("KEGG", "mmu_KEGG", "Reactome", "BioCarta", "cell_markers",
    ...
    expect_is(gset_obj <- fetch_gene_sets(gene_sets = gset_name, min_gset_size = 10,
  ...
  # Custom
  gset_obj <- fetch_gene_sets(gene_sets = "Custom", min_gset_size = 20, max_gset_size = 200,
...
test_that("`fetch_gene_sets()` -- min/max_gset_size args correctly filter gene sets",
          {
            ...
            for (idx in seq_along(min_max_pairs)) {
              ...
              expect_is(gset_obj <- fetch_gene_sets(gene_sets = "KEGG", min_gset_size = cur_vals["min"],
...
test_that("`fetch_gene_sets()` -- for 'Custom' gene set, check if the custom objects are provided",
          {
            ...
            expect_error(fetch_gene_sets(gene_sets = "Custom", custom_genes = kegg_genes),
                         "`custom_genes` and `custom_descriptions` must be provided if `gene_sets = \"Custom\"`")
            expect_error(fetch_gene_sets(gene_sets = "Custom", custom_descriptions = kegg_descriptions),
                         "`custom_genes` and `custom_descriptions` must be provided if `gene_sets = \"Custom\"`")
          })
...
test_that("`fetch_gene_sets()` -- argument checks work", {
  ...
  expect_error(fetch_gene_sets(gene_sets = "INVALID"), paste0("`gene_sets` should be one of ",
  ...
  expect_error(fetch_gene_sets(gene_sets = "Custom", custom_genes = "INVALID", custom_descriptions = ""),
               "`custom_genes` should be a list of term gene sets")
  expect_error(fetch_gene_sets(gene_sets = "Custom", custom_genes = list(), custom_descriptions = ""),
