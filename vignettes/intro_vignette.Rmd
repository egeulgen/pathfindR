...
> During input processing, gene symbols that are not in the PIN are identified and exluded. For human genes, if aliases of these missing gene symbols are found in the PIN, these symbols are converted to the corresponding aliases (controlled by the argument `convert2alias`). This step is performed to best map the input data onto the PIN.
...
> It is also possible to run pathfindR using non-human organism annotation. For this, `convert2alias` must be set to `FALSE`, the custom gene set objects AND the path to a custom PIN file belonging to the species of interest must be specified.
