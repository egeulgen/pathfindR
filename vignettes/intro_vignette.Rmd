...
By the default, `term_gene_heatmap()` visualizes the heatmap for the top 10 enriched terms:

```{r hmap1, eval=TRUE, fig.height=4, fig.width=8}
term_gene_heatmap(RA_output)
```

To plot all of the enriched terms in the enrichment results data frame, set `num_terms = NULL` (not advised due to cluttered visualization):

```{r hmap2, eval=TRUE, fig.height=4, fig.width=8}
...
```{r hmap3, eval=TRUE, fig.height=4, fig.width=8}
term_gene_heatmap(RA_output, use_description = TRUE)
```


As explained above, if you supply the input genes data frame to `term_gene_heatmap()`, the tiles are colored according to the change values:

```{r hmap4, eval=TRUE, fig.height=4, fig.width=8}
...
```{r upset2, eval=TRUE, fig.height=4, fig.width=8}
UpSet_plot(RA_output, num_terms = 25)
```

Again, to plot using full term names (instead of IDs which is the default), set `use_description = TRUE`:

```{r upset3, eval=TRUE, fig.height=4, fig.width=8}
