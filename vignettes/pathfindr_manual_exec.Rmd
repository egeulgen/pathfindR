...
This vignette walks through each step of the pathfindR active-subnetwork-oriented pathway enrichment analysis. The wrapper function `run_pathfindR` can be used to perform this analysis from start to end but for users who wish to have further control over the workflow, the following script will be much more useful.
...
For active subnetwork search, we use the function `active_snw_search` and for enrichment analyses we use `enrichment_analyses`. Because the active subnetwork search algorithms are stochastic, we iterate the search and enrichment steps multiple times (here we are using a regular `for` loop but in the wrapper function `run_pathfindR` a parallel loop using `foreach` is used), and later summarize the results.
...
for (i in 1:n_iter) {
  ...
  ###### Enrichment analyses
  enrichment_res <- enrichment_analyses(active_snws, 
                                        gene_sets = "GO-All", # using "GO-All" as our gene sets for enrichment
                                        pin_path = pin_path,
                                        input_genes = RA_processed$GENE,
                                        list_active_snw_genes = TRUE) # also listing the non-input active snw genes in output (default = FALSE)
...
final_res <- summarize_enrichment_results(combined_res, 
                                          list_active_snw_genes = TRUE)

##### Annotate DEGs Involved in Each Pathway
final_res <- annotate_pathway_DEGs(final_res, 
...
Finally, we can visualize each pathway diagram (in this case graphs of interactions of pathway-involved genes for each pathway, see `?visualize_pws` for more details):

```{r vis_pws, eval=FALSE}
visualize_pws(final_res, 
              RA_processed, 
              gene_sets = "non-KEGG", # this can either be "KEGG" (default) or non-KEGG
