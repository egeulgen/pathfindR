...
<img src="./pathfindr.png" width="650"/>
...
# Obtain the `/path/to/protein-protein/interaction/network`
...
> An active subnetwork can be defined as a group of interconnected genes in a protein-protein interaction network (PIN) that predominantly consists of significantly altered genes. In other words, active subnetworks define distinct disease-associated sets of interacting genes.

We obtain the full path to the PIN file to be used during analyses via `return_pin_path`. The `pin_name_path` argument can be one of "Biogrid", "GeneMania", "IntAct", "KEGG" or it can be the path to a custom PIN file provided by the user.

```{r path2PIN}
path2PIN <- return_pin_path(pin_name_path = "Biogrid")
```
...
RA_processed <- input_processing(input = RA_input, # the input: in this case, differential expression results
                                 ...
                                 pin_path = path2PIN, # path/to/PIN/file
                                 human_genes = TRUE) # boolean indicating whether the input genes are human gene symbols
```

> After checking that the data frame complies with the requirements, `input_processing` filters the input so that genes with p values larger than `p_val_threshold` are excluded. Next, if human genes are provided (controlled by the argument `human_genes`), gene symbols that are not in the PIN are identified. If aliases of these missing gene symbols are found in the PIN, these symbols are converted to the corresponding aliases. This step is performed to best map the input data onto the PIN.
...
for (i in 1:n_iter) {
  ...
  active_snws <- active_snw_search(input_for_search = RA_processed, 
                                   ...
                                   sig_gene_thr = 10, # you may tweak these arguments for optimal filtering of subnetworks
                                   search_method = "GR")
...
We next summarize the enrichent results (in `combined_res`) using `summarize_enrichment_results` and annotate the involved significant (input) genes in each term using `annotate_term_genes`.
...
Finally, we can visualize each enriched term diagram using `visualize_terms`. In this case, these will be graphs of interactions of pathway-involved genes for each pathway. See `?visualize_terms` for more details.
...
visualize_terms(result_df = final_res, 
                ...
                pin_name_path = path2PIN)
```

<img src="./example_interaction_vis.png" width="650" height="519.87" />
...
<img src="./man_enrichment_chart.png" width="650"/>
