...
We obtain the necessary gene sets for enrichment analyses using `fetch_gene_sets()`:

``` {r gene_set}
# using "BioCarta" as our gene sets for enrichment
biocarta_list <- fetch_gene_sets(
  gene_sets = "BioCarta",
  min_gset_size = 10,
  max_gset_size = 300
)
...
for (i in 1:n_iter) {
  ...
  active_snws <- get_active_subnetworks(
    ...
    seed_for_stochastic_methods = i # setting seed to ensure reproducibility per iteration
  )
