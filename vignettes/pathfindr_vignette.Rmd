...
`pathfindR` is an R package for pathway enrichment analysis of gene-level omics data utilizing active subnetworks. The package also enables clustering of the enriched pathways, scoring of overall pathway activity and visualization of analysis results. pathfindR is described in detail in _Ulgen E, Ozisik O, Sezerman OU. 2019. pathfindR: An R Package for Comprehensive Identification of Enriched Pathways in Omics Data Through Active Subnetworks. Front. Genet. [https://doi.org/10.3389/fgene.2019.00858](https://doi.org/10.3389/fgene.2019.00858)_
...
Useful arguments to control the certain aspects of analysis are provided below:
```{r eval=FALSE}
...
# available gene sets are KEGG, Reactome, BioCarta, GO-All, GO-BP, GO-CC and GO-MF
# default is KEGG
...
RA_output <- run_pathfindR(RA_input, search_method = "GA")
...
# to report the non-DEG active subnetwork genes (for further analysis)
...
Next, pathway enrichment analyses are performed using the genes in each of the active subnetworks. For this, up-to-date information on human gene sets from KEGG, Reactome, BioCarta and Gene Ontology were retrieved and is available for use within the package. The user may specify custom gene sets, including gene sets for non-human organisms, as described in the section "pathfindR Analysis with Custom Gene Sets" below.
...
- (non_DEG_Active_Snw_Genes (OPTIONAL): the non-DEG active subnetwork genes involved in the pathway)
...
<img src="./pw_clustering.png" style="max-width:100%;" />
...
```{r term_gene1, fig.height=4, fig.width=8}
...
```{r term_gene2, fig.height=4, fig.width=8}
...
```{r term_gene3, fig.height=4, fig.width=8}
term_gene_graph(RA_output, num_terms = 3, use_names = TRUE)
```
...
```{r term_gene4, fig.height=4, fig.width=8}
...
custom_result <- run_pathfindR(input,
                               ...
                               output_dir = "misc_data/CREB_MYC")
```

``` {r custom_out, include = FALSE}
data(custom_result, package = "pathfindR")
```
