...
By default the node sizes are plotted proportional to the number of genes a term contains (`num_genes`). To adjust node sizes using the $-log_{10}$(lowest p values), set `term_size = "p_val"`:

```{r term_gene4, eval=FALSE}
create_term_gene_graph(example_pathfindR_output, num_terms = 3, term_size = "p_val")
```

`create_term_gene_graph` can also take the `genes_df` as input to visualise fold-changes, Fold Enrichments of the term nodes and an up-set like plot in graph context.

```{r term_gene5}
g <- create_term_gene_graph(
  result_df = example_pathfindR_output,
  genes_df = example_pathfindR_input,
...
See `?create_term_gene_graph` for more details.
