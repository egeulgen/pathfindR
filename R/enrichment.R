...
#' )
enrichment <- function(
  input_genes, genes_by_term = pathfindR.data::kegg_genes, term_descriptions = pathfindR.data::kegg_descriptions,
  ...
  A <- vapply(
    gset_for_fe, function(gset) length(intersect(sig_genes_vec, gset)),
    1L
  ) / length(sig_genes_vec)
  B <- vapply(
    gset_for_fe, function(gset) length(intersect(background_genes, gset)),
    1L
...
#'   pin_name_path = "KEGG"
#' )
enrichment_analyses <- function(
  snws, sig_genes_vec, pin_name_path = "Biogrid", genes_by_term = pathfindR.data::kegg_genes,
  ...
  enrichment_res <- lapply(snws, function(x) {
    enrichment(
      input_genes = base::toupper(x), genes_by_term = genes_by_term,
...
summarize_enrichment_results <- function(enrichment_res, list_active_snw_genes = FALSE) {
  ...
  ## reorder columns
  keep <- c(
    "ID", "Term_Description", "Fold_Enrichment", "occurrence", "support",
    "lowest_p", "highest_p"
  )
  if (list_active_snw_genes) {
