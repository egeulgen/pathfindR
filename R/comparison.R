...
combine_pathfindR_results <- function(result_A, result_B, plot_common = TRUE) {
  combined_df <- merge(result_A, result_B,
...
combined_results_graph <- function(
  combined_df, selected_terms = "common", use_description = FALSE,
  ...
  necessary_cols <- c(
    ID_column, "combined_p", "Up_regulated_A", "Down_regulated_A",
  ...
  for (i in base::seq_len(nrow(combined_df))) {
    up_genes <- c(unlist(strsplit(combined_df$Up_regulated_A[i], ", ")), unlist(strsplit(
      combined_df$Up_regulated_B[i],
      ", "
    )))
    down_genes <- c(
  ...
  up_genes_A <- unlist(lapply(combined_df$Up_regulated_A, function(x) {
    unlist(strsplit(
      x,
      ", "
    ))
  }))
  down_genes_A <- unlist(lapply(combined_df$Down_regulated_A, function(x) {
    unlist(strsplit(
      x,
      ", "
    ))
  }))
  up_genes_B <- unlist(lapply(combined_df$Up_regulated_B, function(x) {
    unlist(strsplit(
      x,
      ", "
    ))
  }))
  down_genes_B <- unlist(lapply(combined_df$Down_regulated_B, function(x) {
    unlist(strsplit(
      x,
      ", "
    ))
  ...
  terms_A <- combined_df[
    !is.na(combined_df$lowest_p_A) & is.na(combined_df$lowest_p_B),
    ID_column
  ]
  terms_B <- combined_df[
    is.na(combined_df$lowest_p_A) & !is.na(combined_df$lowest_p_B),
  ...
  create_graph <- function(g, for_coloring, size) {
    ...
    vertex_cols <- c(
      `Common term` = "#FCCA46", `A-only term` = "#9FB8AD", `B-only term` = "#619B8A",
