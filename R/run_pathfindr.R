...
#' }
run_pathfindR <- function(
  input, gene_sets = "KEGG", min_gset_size = 10, max_gset_size = 300,
  ...
  gset_list <- fetch_gene_sets(
    gene_sets = gene_sets, min_gset_size = min_gset_size,
  ...
  final_res <- annotate_term_genes(
    result_df = final_res, input_processed = input_processed,
